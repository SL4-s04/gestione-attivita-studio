...
void userGui(); //generate a gui
void masterGui(); //racchiude tutte le funzioni inserite
void userInput(char userinput); //for listen input by keyboard

void userGui() {

system("cls"); //for updating 

printBanner(); //print a banner
infoText(); //for generate a init text
...
void userInput(char userinput) {
    ...
    case ' ': if(axis.y == 1 ) { masterGui(); } else if(axis.y == 2) { exit(1); } break; //select
...
void masterGui() {
	...
	newList(s); //create a newList setupping all par to 0 or NULL
	
	printf("insert a number: ");
    scanf("%d", &n.intero); //pre-condition
    //the begin value haves impact on all functions
    
    inputChar(c, desc, n); //for taking in input strings
    
    s = addTail(s, n); //for taking in input numbers/elements
    printf("---------------\n");
    
    sorting(s, n); //sorting of the elements
    printTail(s, priority, n);
    
    printf("---------------\n");
    printf("WARNING:u cant adding a year value below to 1000!\n");
    addDate(day, month, year, n); //take in input 3 values
	
	showDetails(s, c, desc, day, month, year, n); //output that prints all details
	printf("---------------\n");
	
	printf("tempo da stimare: ");
	scanf("%d", &time);
	
	s = alterElements(s, n); //modify the elements value
	printf("---------------\n");
	
	showWeek(s, c, day, month, year, n, time); //prints in output the weekly reports
